...
def create_task(task :Create_Task_Basemodel):
    try:
        task_utils.get_valid_input(task.title)
        task_utils.get_valid_input(task.description)
        task_utils.validate_date(task.due_date)
        task_utils.validate_status(task.status)
        if task.priority:
            task_utils.validate_priority(task.priority)
        ...
        return new_task.__dict__
            ...
            return {"message": f"An error occurred while creating the task.{e}"}
...
def update_task(task_id: uuid.UUID, task_update: Update_Task_Basemodel):
    try:
        ...
        if task_update.due_date is not None:
            task_utils.validate_date(task_update.due_date)
            task_to_update.due_date = task_update.due_date
        if task_update.status is not None:
            task_utils.validate_status(task_update.status)
...
def get_all_tasks():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"An error occurred while retrieving tasks:\n{e}")
...
def get_task_by_id(task_id: uuid.UUID):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"An error occurred while retrieving the task:\n{e}")
...
def delete_task_by_id(task_id: uuid.UUID):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"An error occurred while deleting the task:\n{e}")
